...
- ✅ **Fusión de pedidos**: si el cliente tiene **cualquier** pedido en Shopify
  (no solo del catálogo) de <48h, abierto, sin enviar y **sin pagar**, se cancela
  (con restock) y sus productos se consolidan en el nuevo para un solo envío. Los
  pedidos **pagados nunca se tocan** (evita reembolsos) y no se cuenta/cobra doble
...
- **Tope del plan Free**: máximo 10 pedidos/mes. Se cuenta por mes en la app y,
  ...
  aplica en Fase 3).
- **Fusión de pedidos**: busca en **todos** los pedidos del cliente en Shopify
  (por teléfono) uno abierto, sin enviar y **sin pagar** de <48h. Si el que se
  fusiona es **nuestro**, se marca `status = 'merged'` (excluido del
  tope/métricas/facturación) y no se vuelve a cobrar; si es **externo**, el
  pedido consolidado cuenta como nuevo. Los pedidos **pagados** nunca se cancelan.
